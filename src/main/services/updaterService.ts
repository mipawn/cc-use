...
async function checkForUpdatesViaGitHub(
  ...
  const response = await fetch(
    "https://api.github.com/repos/mipawn/cc-use/releases/latest",
    {
  ...
  if (useElectronUpdater) {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error("electron-updater download timeout")),
          DOWNLOAD_TIMEOUT,
        ),
      );
      ...
      console.warn(
        "electron-updater download failed, falling back to manual download:",
        err,
      );
...
export async function installUpdate(): Promise<string | null> {
  ...
  console.error(
    "No downloaded file found, downloadedFilePath:",
    downloadedFilePath,
  );
