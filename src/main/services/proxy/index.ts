...
export async function startProxy(): Promise<void> {
  ...
  app.use("/", async (req: Request, res: Response, next: NextFunction) => {
    ...
    const sessionToken = apiKeyValue.startsWith("session-")
      ? apiKeyValue
      : null;
    ...
    if (!sessionToken) {
      res.status(401).json({
        ...
        message:
          "Please launch the project from CC-Use to create a valid session",
      });
      return;
    }
    ...
    if (!session) {
      res.status(401).json({
        ...
        message:
          "Session expired or invalid. Please relaunch the project from CC-Use",
      });
      return;
    }
    ...
    console.log(
      `[Proxy] Provider: found=${!!provider}, baseUrl=${provider?.baseUrl}`,
    );
    if (!provider) {
        ...
        on: {
          ...
          proxyRes: (proxyRes) => {
            ...
            if (statusCode === 401 || statusCode === 429) {
              console.log(
                `Request failed with ${statusCode} for key ${apiKey.alias || apiKey.id}`,
              );
      ...
      const proxy = createProxyMiddleware({
        ...
        on: {
          proxyReq: (proxyReq) => {
            ...
            proxyReq.setHeader("x-api-key", apiKey.value);
            console.log(
              `[Proxy] Forwarding: ${proxyReq.method} ${targetOrigin}${proxyReq.path}`,
            );
          },
