...
import { Modal, Form, Input, Select, Typography, Space, Tag, Divider } from 'antd'
import { FolderOutlined, KeyOutlined, CloudServerOutlined } from '@ant-design/icons'
...
export default function NewProjectModal({
  ...
  const filteredApiKeys = useMemo(() => {
    ...
    return apiKeys.filter(
      (k) => k.providerId === selectedProviderId && k.isActive && !k.isExhausted,
    )
      ...
      .map((p) => {
        ...
        return {
          ...
          label: (
            <Space>
              <span>{p.name}</span>
              <Tag color={keyCount > 0 ? 'blue' : 'default'} className={styles.keyCountTag}>
                {keyCount} keys
              </Tag>
            </Space>
          ),
  ...
  return (
    ...
    >
      ...
      <Form form={form} layout='vertical' className={styles.form}>
        ...
        >
          <Input placeholder={t('newProject.myProject')} size='large' className={styles.input} />
        </Form.Item>
            ...
            notFoundContent={
              <div className={styles.emptyKeys}>
                <Text type='secondary'>
                  {selectedProviderId
                    ? t('apiKeys.noKeys')
                    : t('newProject.selectProviderFirst') || '请先选择供应商或直接选择密钥'}
                </Text>
              </div>
            }
          />
