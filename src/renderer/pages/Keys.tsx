...
export default function Keys() {
  ...
  const [refreshingKeyUsageIds, setRefreshingKeyUsageIds] = useState<
    Set<string>
  >(new Set());
  ...
  const [proxySessionToken, setProxySessionToken] = useState<string | null>(
    null,
  );
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <div>
          ...
          <Text type="secondary">
            {t("keys.subtitle") || "管理您的 API 密钥和供应商"}
          </Text>
        </div>
        <Space>
          <Button
                      ...
                      {provider.walletBalanceType !== "none" && (
                        <Tooltip title={t("providers.refreshBalance")}>
                          <Button
                            ...
                            icon={
                              <ReloadOutlined
                                spin={refreshingIds.has(provider.id)}
                              />
                            }
                        ...
                        >
                          ...
                          <div className={styles.statsRow}>
                            ...
                            <div className={styles.statItem}>
                              ...
                              <span className={styles.statValue}>
                                {t("keys.todayCost") || "今日"}: $
                                {(keyCostStats[key.id]?.todayCost || 0).toFixed(
                                  4,
                                )}
                              </span>
                            </div>
                            ...
                            <div className={styles.statItem}>
                              ...
                              <span className={styles.statValue}>
                                {t("keys.totalCost") || "累计"}: $
                                {(keyCostStats[key.id]?.totalCost || 0).toFixed(
                                  4,
                                )}
                              </span>
                            </div>
