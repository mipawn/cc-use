...
describe('Provider Type Config', () => {
  ...
  describe('generateTerminalCommand', () => {
    it('should generate correct command for claude provider in direct mode', () => {
      const provider = { type: 'claude' as ProviderType, baseUrl: 'https://api.anthropic.com' }
      const command = generateTerminalCommand(provider, 'sk-test-key')

      expect(command).toBe('export ANTHROPIC_BASE_URL=https://api.anthropic.com && export ANTHROPIC_API_KEY=sk-test-key && claude')
    })

    it('should generate correct command for codex provider in direct mode', () => {
      const provider = { type: 'codex' as ProviderType, baseUrl: 'https://api.openai.com' }
      const command = generateTerminalCommand(provider, 'sk-openai-key')

      expect(command).toBe('export OPENAI_BASE_URL=https://api.openai.com && export OPENAI_API_KEY=sk-openai-key && codex')
    })
    ...
    it('should generate correct command in proxy mode', () => {
      ...
      expect(command).toBe('export ANTHROPIC_BASE_URL=http://localhost:12345 && export ANTHROPIC_API_KEY=proxy && claude')
    })
