...
- **上传图标经 IPC 读取**：新增 `icon_read` 命令与统一的 `ProviderIcon` 组件，替换列表、页面与弹窗里各自为政的图标渲染；开发 WebView 下自定义协议不生效的问题随之消失
- **供应商卡片与密钥列表重排**：账户金额内联显示、成功率改为文本、低频操作收进「更多操作」；密钥改为双列网格并新增「添加密钥」卡片
- **查询模板入口**：供应商编辑器可直接选用预设或示例查询脚本，只替换脚本，不动模型映射等其它配置

### Changed

- 额度查询缺少 API 密钥 / 账户令牌 / 用户 ID 时不再发请求，直接说明缺什么；界面同步禁用刷新并给出原因
- 查询报错、`isValid: false`、空结果与写库失败都不再覆盖已有缓存；脚本只给周期 ID 时用 ID 补标签
- Go 周期固定按 5 小时 → 每周 → 每月显示（此前是响应键的字母序），已用百分比按查询返回值显示，不再取整
- Auto 记录抽屉加载失败时给出可读提示与重试入口

### Fixed

- 工具筛选带时间范围时拼出的 SQL 非法，Auto 记录按工具名筛选会直接报错
- 手动触发的构建只产出一次性测试版（`test-{run_id}`，Pre-release），不写更新清单，也不占用正式 tag

### Compatibility

- 3.10.0 的 Release 只有 Apple Silicon 包（Intel 构建失败），本版补齐 arm 与 Intel 两个包
- 升级后无需重新配置；本版不改写已保存的账户查询脚本

## [3.10.0] - 2026-09-11

本版把供应商从「地址 + 密钥」扩展成一套可编辑的账户配置，并重做了控制台与用量统计。

### Added

- **供应商预设**：自定义 / DeepSeek / New API / OpenCode Go 四个模板，一键填入地址、账户查询与新建密钥的默认配置。预设只负责预填，保存后每个字段都可改
- **账户查询改为可写的 JavaScript**：一段脚本同时描述「请求什么」和「怎么读」，返回 `remaining` / `windows` / `isValid` 等标准字段。DeepSeek 的余额与 OpenCode Go 的计量周期是同一套字段的两种填法。脚本在沙箱里执行（16MB 内存 / 5 秒），保存前会校验但不发送
- **密钥级额度查询**：脚本预填、由开关控制，默认关闭
- **供应商请求头**：为这家供应商的推理流量追加请求头，只增不改，支持 `{{baseUrl}}`
- **控制台**：请求 / 异常 / 运行日志三个视图，daemon 与 app 的日志落到本地滚动文件，刷新或重启后能恢复
- **Auto mode 记录**：分类器审核了什么、模型返回什么、请求最后怎样，分别记录；统计页可查
- **逐页刷新**：⌘R 刷新当前页，不再整页重载

### Changed

- 供应商与密钥编辑弹窗重排：预设改为名称胶囊，高级设置只留代理与账户查询
- 用量统计：缓存命中率与延迟并入顶部指标，模型用量图紧随其后，按 Key / 按项目归因放最后
- 状态栏按会话的实际工作目录显示项目名

### Fixed

- **Codex 接管恢复**改为字段级还原：原先用首次接管前的快照整份覆盖，会连同期间新增的 MCP 服务器、插件与项目信任一起丢掉
- 余额与额度查询改为以保存的请求为准，此前部分分支忽略保存的地址与请求头；变量替换也在各分支间统一
- 图标来源收敛为单一模块，修复自定义预设显示为一张加载不出来的图
- 新建密钥默认开启 Auto mode 分类器适配，避免分类器按用户没选的模型作答
- 复制密钥保留凭据与完整配置

### Compatibility

- 升级后首次启动会把存量供应商与密钥的账户查询翻译成等价的脚本，保留各自实际在用的地址；不需要重新配置
- 余额与额度查询现在要求 HTTPS，且必须指向该供应商自己的域名
...
- Claude Code 新增按 Key 启用的 Auto mode 分类器适配：可指定同一供应商下的分类器模型，或留空跟随同一会话最近一次携带工具请求所使用的上游模型
- 分类器思考强度支持「低」「关闭思考」「保持原样」；低思考设置 1024 token 思考预算及 low effort，并为短分类请求预留判定输出空间

### Compatibility

- 此功能默认关闭，仅改写已识别的 Claude Code 权限分类请求；保留分类器提示词、停止序列和上游判定响应，不改变普通会话和其他客户端的请求行为
- 请求识别基于 Claude Code 2.1.220；尚未识别会话模型时回退到既有模型映射。不同代理线路、Key 和 Claude 会话的最近模型分别缓存
- 该适配仍属实验性功能：已通过模拟上游和界面验证，尚未完成真实 DeepSeek、GLM 端到端测试。供应商不支持 low 参数时，可选择关闭思考或保持原样
...
- 新增供应商弹窗移除大尺寸 DeepSeek / 自定义快捷卡片，打开后直接进入通用供应商表单；DeepSeek 图标、余额类型及已有配置继续保留
- DeepSeek 官方供应商改为仅按 `api.deepseek.com` 域名识别，图标回归纯展示用途；中转域名继续按普通供应商处理，页面“内置”标记同步改为“官方”
- 供应商用量统计入口移到启用开关之后，与余额刷新、模型列表和设置等操作统一排列

### Fixed

- macOS 托盘跨日且当日暂无请求时显式写入空标题，解决底层把 `set_title(None)` 当作不更新、导致昨日 Token 一直保留到下一笔请求的问题
...
- 供应商和密钥卡新增多折线用量趋势，支持近一周、近一月、近一年、本月、上月、本年和按天自定义日期范围
- 线路摘要新增平均首 Token 时间，供应商级汇总全部密钥，密钥级保持独立归因
- 全局用量统计新增模型用量堆叠柱状图，按日 / 周 / 月粒度展示每个时间桶内各模型的 Token 构成，悬停可查看模型与用量，点击图例可显隐模型

### Changed

- 供应商 / 密钥质量统计的请求总数包含无 Token 的失败请求，使成功率使用完整且可解释的分母
- 资源趋势改用真实单位坐标轴；Token 构成包含输入、输出、缓存读取和缓存创建，线路质量包含成功率、缓存命中率与延迟；图例可逐条显隐，请求数仅在悬停中作为上下文展示
- 趋势数据按所选跨度在数据库中动态聚合：短周期按日、年度按周、超长周期按月，不再固定抽样并丢失中间峰值
- 供应商 / 密钥用量弹窗将六项摘要指标压缩为单行紧凑卡片，提升有限宽度内的信息密度

### Removed

- 全局用量统计移除“每日使用趋势”“失败明细”和 Token 构成，保留模型用量、Key / 项目归因、摘要及最近请求记录

### Fixed

- 托盘后台刷新重新与菜单重建解耦，并主动检测本地自然日切换；无新请求且窗口隐藏时，昨日 Token 也会在 30 秒内清空
- Claude Code 状态栏会正确引用包含空格的应用路径；第三方配置覆盖后必须通过回读校验才提示启用成功
- 全局日期组件同步 Ant Design 与 Day.js 语言，中文界面不再出现英文月份、星期或操作文案
- 开发启动先准备 sidecar 再启动 Tauri，消除两个 Cargo 并发造成的锁等待；无 Rust 变更时跳过 sidecar 编译，`Ctrl+C` 会统一结束 Tauri、桌面端和 Vite
