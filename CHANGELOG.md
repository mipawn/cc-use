...
- 供应商和密钥卡新增多折线用量趋势，支持本周、上周、本月、上月、近一周、近一个月、全部和自定义日期范围
- 线路摘要新增平均首 Token 时间，供应商级汇总全部密钥，密钥级保持独立归因

### Changed

- 供应商 / 密钥质量统计的请求总数包含无 Token 的失败请求，使成功率使用完整且可解释的分母
- 全局和资源趋势改用真实单位坐标轴，按 Token 构成、请求与缓存、线路质量组织可比较指标；图例可逐条显隐，悬停显示当日全部可见曲线的实际值
...
- 开发模式由受 Tauri 生命周期管理的单一进程启动 Vite，退出或异常中断后不再遗留进程占用 5173
