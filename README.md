...
- **本地 daemon 服务** - 常驻本地网关，通过 session token 中转请求，开启后可实现费用追踪和热切换
- **费用追踪** - daemon 模式下自动记录每次请求的 Token 用量和费用
...
在「项目」页面或「设置」页面可以开关本地 daemon 服务。服务开启后：

- 请求经过本地服务中转，使用 session token 替代真实密钥
...
更多测试入口（如 `pnpm test:web`、`pnpm test:rust`、`pnpm test:all`）见 [docs/testing/README.md](./docs/testing/README.md)。
